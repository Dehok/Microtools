...
export default function AvifToJpgPage() {
    ...
    async (file: File): Promise<ConvertedImage> => {
      return new Promise((resolve, reject) => {
        ...
        image.onload = () => {
          ...
          canvas.toBlob(
            (blob) => {
              URL.revokeObjectURL(sourceUrl);
              if (!blob) {
                reject(new Error(`Could not convert ${file.name}.`));
                return;
              }
              resolve({
                original: file,
        ...
        image.onerror = () => {
          URL.revokeObjectURL(sourceUrl);
          reject(
            new Error("AVIF decoding failed in this browser. Try a Chromium-based browser or newer Safari.")
          );
        };
  ...
  const handleFiles = useCallback(
    async (files: FileList | File[]) => {
      setError("");
      const input = Array.from(files).filter(isAvifFile);
      if (!input.length) {
        setError("Please upload AVIF files.");
        return;
      }
  ...
  const download = (item: ConvertedImage) => {
    ...
    link.download = item.original.name.replace(/\.avif$/i, "") + ".jpg";
  ...
  return (
    <ToolLayout
      ...
      description="Convert AVIF images to JPG (JPEG) in your browser with quality control and no uploads."
      relatedTools={["webp-to-jpg", "image-format-converter", "image-compressor"]}
    >
      <div className="mb-4">
      ...
      >
        <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
          {processing ? "Converting..." : "Drop AVIF files here or click to upload"}
        </p>
      ...
      <div className="mt-12 space-y-6 border-t border-gray-200 dark:border-gray-700 pt-8 text-sm leading-relaxed text-gray-600 dark:text-gray-400">
        ...
        <p>
          AVIF offers strong compression but some tools still require JPG. This converter decodes AVIF in the browser,
          re-encodes to JPG, and lets you control output quality before download.
        </p>
        ...
        <div className="space-y-4">
          <details className="group">
            <summary className="cursor-pointer font-medium text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400">
              Why can AVIF conversion fail in some browsers?
            </summary>
            <p className="mt-2 pl-4 text-gray-600 dark:text-gray-400">
              AVIF decoding support depends on browser version and platform codecs. Newer Chromium and Safari releases
              usually work best.
            </p>
          </details>
          <details className="group">
            ...
            <p className="mt-2 pl-4 text-gray-600 dark:text-gray-400">
              No. Processing is local in your browser, aligned with your privacy policy.
            </p>
          </details>
        </div>
      </div>
