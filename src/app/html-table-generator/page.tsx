    ...
    >
      ...
      <div className="mb-5 grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-3">
        <div>
          <label className="mb-1 block text-xs font-medium text-gray-600">
            Rows <span className="text-gray-400">(1–50)</span>
          </label>
          <input
            type="number"
            min={1}
            ...
            className="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-sm focus:border-blue-500 focus:outline-none"
          />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-gray-600">
            Columns <span className="text-gray-400">(1–20)</span>
          </label>
          <input
            type="number"
            min={1}
        ...
        <table className="w-full text-sm">
            ...
            <thead>
              <tr>
                {Array.from({ length: cols }, (_, c) => (
                  <th key={c} className="px-1 py-1 bg-blue-50">
                    <input
                      type="text"
                      value={data[0]?.[c] ?? ""}
                      onChange={(e) => updateCell(0, c, e.target.value)}
                      placeholder={`Header ${c + 1}`}
                      className="w-full min-w-[80px] rounded border border-blue-200 bg-white px-2 py-1 text-sm font-semibold focus:border-blue-500 focus:outline-none"
                    />
                  </th>
                ))}
              </tr>
          ...
          <tbody>
            {Array.from({ length: rows - startRow }, (_, rowIdx) => {
              ...
              return (
                <tr key={r} className={striped && rowIdx % 2 !== 0 ? "bg-gray-50" : "bg-white"}>
                  {Array.from({ length: cols }, (_, c) => (
                    <td key={c} className="px-1 py-0.5 border-b border-gray-100">
                      <input
                        type="text"
                        value={data[r]?.[c] ?? ""}
                        onChange={(e) => updateCell(r, c, e.target.value)}
                        placeholder={`Cell ${r + 1},${c + 1}`}
                        className="w-full min-w-[80px] rounded border border-gray-200 bg-gray-50 px-2 py-1 text-sm focus:border-blue-500 focus:outline-none"
                      />
                    </td>
                  ))}
                </tr>
      ...
      <div>
        <div className="mb-2 flex items-center justify-between">
          <label className="text-sm font-medium text-gray-700">Generated HTML</label>
          <CopyButton text={generatedHtml} />
        </div>
        <pre className="overflow-x-auto rounded-lg border border-gray-200 bg-gray-50 p-3 font-mono text-xs leading-relaxed">
          {generatedHtml}
        </pre>
      </div>
      ...
      <div className="mt-8 border-t border-gray-200 pt-6 text-sm text-gray-600">
        <h2 className="mb-3 text-lg font-semibold text-gray-900">What Is an HTML Table?</h2>
        <p className="mb-4">
          An HTML table is a structured grid of rows and columns defined with the{" "}
          <code className="rounded bg-gray-100 px-1 py-0.5 font-mono text-xs">&lt;table&gt;</code>,{" "}
          <code className="rounded bg-gray-100 px-1 py-0.5 font-mono text-xs">&lt;tr&gt;</code>,{" "}
          <code className="rounded bg-gray-100 px-1 py-0.5 font-mono text-xs">&lt;th&gt;</code>, and{" "}
          <code className="rounded bg-gray-100 px-1 py-0.5 font-mono text-xs">&lt;td&gt;</code>{" "}
        ...
        <h2 className="mb-3 text-lg font-semibold text-gray-900">Responsive Tables</h2>
        <p>
          Enabling the responsive wrapper adds{" "}
          <code className="rounded bg-gray-100 px-1 py-0.5 font-mono text-xs">overflow-x: auto</code>{" "}
